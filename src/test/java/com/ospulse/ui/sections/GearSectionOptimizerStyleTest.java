...
public class GearSectionOptimizerStyleTest
{
	 ...
	 * still asserted here.
	 *
	 * <p><b>What changed is the fallback</b> (P2 finding, {@code
	 * GearSection#addVariantPlainForm}): the original version asserted the
	 * plain base id came back <i>owned and free</i> — "that's the whole point
	 * of the mapping". It is not, and that assertion encoded a defect. The
	 * credit's entire justification is that the player effectively HAS the
	 * plain item because they hold the variant. Excluding the variant
	 * withdraws that backing, and a player who fortified their Masori mask
	 * has no plain Masori mask in the bank — so recommending one at zero
	 * spend proposes an item that does not exist anywhere, and the bank
	 * highlighter then points at an id that cannot be found. The exclusion
